...
<head>
    ...
    <link rel="stylesheet" href="{{ asset('css/Admin/candidats/edit-profile.css') }}">
</head>
<body>
        ...
        <main class="profile-content">
            ...
            <section id="profile" class="content-section active">
                    ...
                    <div class="profile-upload">
                        <div class="current-picture">
                            ...
                            @if ($candidat->personal_picture_path && file_exists(public_path($personalPicturePath)))
                                <img draggable="false" id="profile-image" src="{{ asset($personalPicturePath) }}" alt="Foto de perfil" class="profile-img">
                            @else
                                <img draggable="false" id="profile-image" src="{{ asset('images/companies_images/100x100.svg') }}" alt="Foto de perfil" class="profile-img">
                            @endif
                        </div>
                        {{-- <form action="{{ route('administration.candidat.upload.profile.picture') }}" method="POST" enctype="multipart/form-data">
                            @csrf
                ...
                <div class="unique-form-container">
                    ...
                    {{-- <form action="{{ route('administration.candidat.update.first.infos') }}" method="post" id="unique-personal-form">
                        @csrf --}}
                        <input type="hidden" name="id_candidat" value="{{$candidat->id}}">
                        <div class="unique-form-row">
                            <div class="unique-form-group">
                                <label class="unique-label" for="unique-firstname">Nombre<span>*</span></label>
                                <input disabled type="text" id="unique-firstname" name="unique-firstname" placeholder="Jake" value="{{$candidat->first_name}}">
                            </div>
                            <div class="unique-form-group">
                                <label class="unique-label" for="unique-lastname">Apellido<span>*</span></label>
                                <input disabled type="text" id="unique-lastname" name="unique-lastname" placeholder="Gyll" value="{{$candidat->last_name}}">
                            </div>
                        </div>
                        <div class="unique-form-row">
                            <div class="unique-form-group">
                                <label class="unique-label" for="unique-phone">Teléfono <span>*</span></label>
                                <input disabled type="tel" id="unique-phone" name="unique-phone" placeholder="+44 1245 572 135" value="{{$candidat->tel}}" required>
                            </div>
                            <div class="unique-form-group">
                                <label class="unique-label" for="unique-email">Correo <span>*</span></label>
                                <input disabled type="email" id="unique-email" name="unique-email" placeholder="Jakegyll@gmail.com" value="{{$candidat->email}}">
                            </div>
                        </div>
                        <div class="unique-form-row">
                            <div class="unique-form-group">
                                <label class="unique-label" for="unique-birthdate">Fecha de nacimiento <span>*</span></label>
                                <input disabled type="date"  value="{{ $candidat->date_of_birth ? \Illuminate\Support\Carbon::parse($candidat->date_of_birth)->format('Y-m-d') : '' }}" id="unique-birthdate" name="unique-birthdate">
                            </div>
                            <div class="unique-form-group">
                                <label class="unique-label" for="unique-gender">Género <span>*</span></label>
                                <select disabled id="unique-gender" name="unique-gender">
                                    <option {{ $candidat->gender == 'male' ? "selected" : ''  }} value="male">Hombre</option>
                                    <option {{ $candidat->gender == 'female' ? "selected" : ''  }} value="female">Mujer</option>
                                </select>
                            </div>
                        </div>
                        ...
                        <div class="unique-form-row">
                            <div class="unique-form-group">
                                ...
                                <input disabled type="text" id="unique-cargo" name="unique-cargo" placeholder="" value="{{$candidat->poste}}" required>
                            </div>
                            <div class="unique-form-group">
                                <label class="unique-label" for="unique-location">Ubicación <span>*</span></label>
                                <input disabled type="text" id="unique-location" name="unique-location" placeholder="" value="{{$candidat->adresse}}" required>
                            </div>
                        </div>
                        ...
                        <div class="job-search-container">
                            ...
                            <div class="job-search-options">
                                <label class="job-search-option">
                                    <input type="radio" name="job-search" value="active" {{ $candidat->priority == 'no' ? "checked" : 'disabled' }}>
                                ...
                                <label class="job-search-option">
                                    <input type="radio" name="job-search" value="open" {{ $candidat->priority == 'yes' ? "checked" : 'disabled' }}>
                                    <div class="job-search-content">
                        ...
                        <div style="display: none;" class="validation-part">
            ...
            {{-- <section id="cv" class="content-section">                 
                ...
                <div class="uploaded-file">                     
                    <span class="file-name">
                        <?php //echo $candidat->cv_file_path ? basename($candidat->cv_file_path) : ''; ?>
                    </span>
                    <a 
                        class="remove-file" 
                        style="<?php //echo $candidat->cv_file_path ? '' : 'display: none;'; ?>"
                        onclick="event.preventDefault(); document.getElementById('delete-document-form').submit();">eliminar</a>                 
                </div>                             
                <div id="pdf-preview-container" style="margin-top: 20px; max-width: 100%; <?php //echo $candidat->cv_file_path ? '' : 'display: none;'; ?>"></div>
                <!-- Formulaire pour envoyer la requête DELETE -->
                <form id="delete-document-form" action="{{ route('administration.candidat.delete.document', ['id' => $candidat->id]) }}" method="POST" style="display: none;">
                    @csrf
                    @method('DELETE')
                </form>
            </section> --}}
            ...
            <section id="cv" class="content-section">                 
                <h2 class="text-15">CV del candidato</h2>                 
                <div class="cv-upload">
                    <a style="text-decoration: none;" href="<?php echo asset('storage/uploads/candidats_documents/' . $candidat->cv_file_path); ?>" download>
                        <label style="display: flex; justify-content: center; gap: 15px; " for="cv-file" class="cv-upload-box">                         
                            <i style="margin-top: 2px; font-size: 21px;" class="fa fa-upload" aria-hidden="true"></i>
                            <span style="font-size: 18px; font-weight: 600;">Descargar</span>                     
                        </label>
                    </a>                     
                </div>                           
                <div class="uploaded-file">                     
            ...
            <section id="notifications" class="content-section">
                ...
                <div class="refresh-email-section">
                    ...
                    <div class="refresh-fields">
                        <div class="current-email">
                                <p class="email-address">{{ $candidat->email }} 
                        ...
                        <div class="update-email-form">
                            ...
                            <form style="max-width: 400px;" method="post" action="{{ route('administration.candidat.edit.email') }}">
                                @csrf
                                <input type="hidden" name="id_candidat" value="{{$candidat->id}}">
                                <input
                                style="width: 100%;"
                ...
                <div class="change-password-section">
                    ...
                    <div class="password-fields">
                        <form style="" method="post" action="{{ route('administration.candidat.edit.password') }}">
                            @csrf
                            <input type="hidden" name="id_candidat" value="{{$candidat->id}}">
                            ...
                            {{-- <div class="add-info">
                                <form action="{{ route('administration.candidat.edit.password') }}" method="POST">
                                    @csrf
                                    <input type="hidden" name="email" value="{{ $candidat->email }}">
                                    <i class="fa-solid fa-circle-info"></i>
                                    <a style="cursor: pointer;" class="forgot-password-link">He olvidado mi contraseña actual</a>
                ...
                <div class="notification-settings">
                    ...
                    <div class="settings-options">
                        {{-- <form method="POST" action="{{ route('administration.candidat.update.preferences') }}"> --}}
                            @csrf
                            <input type="hidden" name="id_candidat" value="{{$candidat->id}}">
                            <div class="option">
                                <label>
                                    <input disabled type="checkbox" name="job_opportunities" value="yes" {{ isset($preferences) && $preferences->job_opportunities === 'yes' ? 'checked' : '' }}>
                            ...
                            <div class="option">
                                <label>
                                    <input disabled type="checkbox" name="newsletter" value="yes" {{ isset($preferences) && $preferences->newsletter === 'yes' ? 'checked' : '' }}>
                            ...
                            <div class="option">
                                <label>
                                    <input disabled type="checkbox" name="privacy_consent" value="yes" {{ isset($preferences) && $preferences->privacy_consent === 'yes' ? 'checked' : '' }}>
                        ...
                        <div class="close-account">
                            ...
                            @else
                                <div class="">
                                    <a style="font-weight: 700" href="#" class="close-account-link" onclick="confirmarActivasion(); return false;">Activar cuenta</a>
                                    <i style="margin-top: 6px;" class="fa-solid fa-circle-info"></i>
                                </div>
                            @endif
    ...
    <script>
        ...
        const existingFilePath = "<?php echo asset('storage/uploads/candidats_documents/' . $candidat->cv_file_path); ?>";
        ...
        if (file && file.type === "application/pdf") {
            ...
            try {
                const response = await fetch("{{ route('administration.candidat.upload.cv') }}", {
                    method: "POST",
                    headers: {
    ...
    <script>
        function confirmarDesactivacion() {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    form.method = 'POST';
                    form.action = '{{route("administration.candidat.disactive.compte", ["id" => $candidat->id])}}';
        ...
        function confirmarActivasion() {
            Swal.fire({
                title: '¿Estás seguro?',
                text: "¿Desea activar esta cuenta de candidato?",
                icon: 'warning',
                showCancelButton: true,
                confirmButtonColor: '#3085d6',
                cancelButtonColor: '#d33',
                ...
                if (result.isConfirmed) {
                    ...
                    const form = document.createElement('form');
                    form.method = 'POST';
                    form.action = '{{route("administration.candidat.active.compte", ["id" => $candidat->id])}}';
