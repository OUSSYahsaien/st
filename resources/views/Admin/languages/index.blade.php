...
@section('content')
    ...
    <head>
        ...
        <link rel="stylesheet" href="{{ asset('css/Admin/settings/languages.css') }}">
    </head>
    <body>
        
        <div class="contai" style="margin-top: -42px;">
    
            <div class="title-h2">Gestión del lenguaje</div>

            <!-- Formulaire d'ajout -->
            <div class="card mb-4">
                <div class="card-body">
                    <form action="{{ route('administration.languages.store') }}" method="POST">
                        @csrf
                        <div class="row">
                            <div class="col-md-6">
                            ...
                            <tbody>
                                @foreach($languages as $language)
                                    <tr data-language-id="{{ $language->id }}">
                                        <td data-label="Description" style="text-align: center;">{{ $language->description }}</td>
        ...
        <script>
            document.querySelectorAll('.btn-danger').forEach(button => {
                button.addEventListener('click', function(e) {
                    ...
                    const languageId = this.closest('tr').getAttribute('data-language-id');
                    ...
                    Swal.fire({
                        ...
                        if (result.isConfirmed) {
                            ...
                            fetch(`/administration/languages/${languageId}`, {
                                method: 'DELETE',
                                headers: {
                            ...
                            .catch(error => {
                                Swal.fire(
                                    'Error',
                                    'Hay un problema al eliminar el idioma.',
                                    'error'
                                );
                            });
                        }
